/**
 * Signals — shared triage UI extracted from routes/signals.tsx.
 *
 * Used by the Signals inbox (routes/signals.tsx) and the per-eye Inbox tab
 * on Opportunities (routes/opportunities.tsx) so both surfaces stay in sync:
 *
 *   - SignalTypeBadge / typeTone — type chip hues
 *   - SignalRow — inbox row (type · source · observed · disposition · actions)
 *   - NewSignalPanel — manual signal entry (T1.12 preview)
 *   - PromoteDialog — promote → JOB opportunity (T1.12)
 */
...
export const typeTone: Record<SignalType, string> = {
...
export function NewSignalPanel({
  open,
  onToggle,
  defaultType,
}: {
...
export function PromoteDialog({ signal, onClose }: { signal: Signal; onClose: () => void }) {
  const queryClient = useQueryClient()
  const { toast } = useToast()
...
export function SignalRow({ s, onDisposition }: {
  s: Signal
